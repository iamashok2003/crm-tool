...
function handleFormSubmit(e) {
  ...
  let data;
  if (state.module === 'biz') {
    const bizName = $('bBizName').value.trim();
    const mobile  = $('bMobile').value.trim();
    if (!bizName) { showToast('Business name is required', 'error'); $('bBizName').focus(); return; }
    if (!mobile)  { showToast('Mobile number is required', 'error'); $('bMobile').focus(); return; }
    ...
    data = {
      ...
      status:           $('bStatus').value || 'Interested',
      notes:            $('bNotes').value.trim(),
      dateAdded:        $('bDateAdded').value
    };
  } else {
    const docName = $('dName').value.trim();
    const mobile  = $('dMobile').value.trim();
    if (!docName) { showToast('Doctor name is required', 'error'); $('dName').focus(); return; }
    if (!mobile)  { showToast('Mobile number is required', 'error'); $('dMobile').focus(); return; }
    ...
    data = {
      ...
      status:       $('dStatus').value || 'Interested',
...
function handleEditSubmit(e) {
  ...
  if (state.module === 'biz') {
    if (!$('ebBizName').value.trim()) { showToast('Business name is required', 'error'); return; }
    if (!$('ebMobile').value.trim())  { showToast('Mobile number is required', 'error'); return; }
    data = {
